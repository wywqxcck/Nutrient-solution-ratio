...
static uint8_t key_sta = KEY_NO;

uint8_t Get_Key_Sta(void)
{
	uint8_t key = key_sta;
	key_sta = 0;
	if(key == KEY_NO && taskIndex != Main_Mune){
		int8_t temp = Get_EC11();
		if(temp == 0){
			key = KEY_NEXT;
		}
		else if(temp == 1){
...
void KEY_GPIO_Init(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOX,ENABLE);//使能PORTA,PORTE时钟

	GPIO_InitStructure.GPIO_Pin  =GPIO_Pin_X;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //PA6设置成输入，默认下拉	  
	GPIO_Init(GPIOX, &GPIO_InitStructure);//初始化GPIOA.6
}
